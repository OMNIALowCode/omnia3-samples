...
function getButton(onClick) {
  ...
  button.setAttribute(
    "class",
    "btn btn-block btn-outline-secondary omnia-file-button"
  );
...
function getFileListEntry(file, disabled, onDownload, onRemove) {
  ...
  listEntry.className =
    "list-group-item d-flex justify-content-between align-items-center p-0 pt-1 pb-1";
...
function getErrorLabel() {
  ...
  errorLabel.className =
    "omnia-file-error-label text-center text-danger d-none pt-2";
  return errorLabel;
}
...
function getFileUploadButton(language, fileInput) {
  ...
  button.innerHTML =
    getTranslation("Upload_Label", language) +
    '&nbsp<i class="fa fa-fw fa-spinner fa-pulse d-none"></i>';
...
function getTranslation(translation, language) {
  const translationSet =
    language != null
      ? TRANSLATIONS[language.toLowerCase()] || TRANSLATIONS.default
...
class OmniaFileUpload extends HTMLElement {
  ...
  deleteFile(file) {
    ...
    return fetch(url, {
      ...
      this._settings.files = this._settings.files.filter(
        (f) => f.name !== file
      );
    ...
    Promise.all(requests)
      ...
      .then((responses) => {
        if (responses.length > 0) {
          const newValue = this._settings.multiple
            ? this._settings.files.map((f) => f.name)
            : [];
          for (const file of this._settings.filesToUpload) {
  ...
  uploadFile(file) {
    ...
    if (file && file.name) {
      ...
      let fileName =
        fileNameSplit.length > 1 ? fileNameSplit.pop() : fileNameSplit[0];
  ...
  endpoint(code) {
    if (
      this._settings.uploadAddress != null &&
      this._settings.uploadAddress !== ""
    )
  ...
  set context(newValue) {
    if (newValue) {
      ...
      this._settings.language =
        this._settings.context.getLanguageTranslator().language;
