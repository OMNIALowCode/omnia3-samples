...
class OMNIAPDFViewerElement extends HTMLElement {
  constructor() {
    ...
    this.render = () => {
      ...
      Promise.all(renderPagesPromises).then((pages) => {
        const pagesHTML = `<div style="width: ${
          this.pageMode > 1 ? '50%' : '100%'
        }; height: '100%'"><canvas></canvas></div>`.repeat(pages.length);
    ...
    //https://codesandbox.io/s/pmy914l2kq?file=/src/index.js:1956-1966
    //https://cdnjs.com/libraries/pdf.js/2.9.359
  ...
  load() {
    ...
    window.initPDFViewer = function (pdfURL) {
      ...
      that.pdfjsLib.getDocument(pdfURL, this.origin, {}).promise.then((pdf) => {
    ...
    if (this.pdfURL) window.initPDFViewer(this.pdfURL);
  }

  showPDF(pdfurl) {
    this.pdfURL = pdfurl;
    if (this.loadFinish) window.initPDFViewer(pdfurl);
  }
  ...
  set value(newValue) {
    ...
    if (this.loadFinish) window.initPDFViewer(newValue);
  }
}
